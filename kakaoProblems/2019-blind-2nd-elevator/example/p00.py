...
def simulator():
    ...
    while True:
        ...
        for i, elevator in enumerate(elevators):
            ...
            start_list, end_list = search_same_dir(passengers, calls, floor, direction)
            if status == 'STOPPED':
                if enter_list or exit_list: action(token, [{'elevator_id': i, 'command': 'OPEN'}])
                elif start_list or end_list: action(token, [{'elevator_id': i, 'command': 'UP' if direction == UP else 'DOWN'}])
                else:
                    # 진행 방향에 더이상 없음, 방향 전환
                    direction *= -1
                    action(token, [{'elevator_id': i, 'command': 'UP' if direction == UP else 'DOWN'}])
            elif status == 'OPENED':
                # 먼저 내리는게 매너
                if exit_list: action(token, [{'elevator_id': i, 'command': 'EXIT', "call_ids": exit_list}])
                elif enter_list: action(token, [{'elevator_id': i, 'command': 'ENTER', "call_ids": enter_list}])
